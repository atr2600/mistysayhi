...
class Robot:
    ...
    def changeImage(self, image_name, timeout=5):
        if image_name in self.images_saved:
            requests.post('http://' + self.ip + '/api/images/change',
    ...
    def moveHead(self, roll, pitch, yaw, velocity=1):
        assert roll in range(-40, 40) and pitch in range(-40, 26) and yaw in range(-81,
                                                                                   81), " moveHead: Roll, Pitch and Yaw needs to be in range -5 to +5"
        assert velocity in range(0, 100), " moveHead: Velocity needs to be in range 0 to 10"
        requests.post('http://' + self.ip + '/api/head',
                      json={"Pitch": pitch, "Roll": roll, "Yaw": yaw, "Velocity": velocity})

    def headRoll(self, roll, velocity=1):
        assert roll in range(-5, 6), " headRoll: Roll needs to be in range -5 to 5"
        requests.post('http://' + self.ip + '/api/head', json={"Axis": "roll", "position": roll, "Velocity": velocity})

    def headPitch(self, pitch, velocity=1):
        assert pitch in range(-5, 6), " headPitch: Pitch needs to be in range -5 to 5"
        requests.post('http://' + self.ip + '/api/head',
                      json={"Axis": "pitch", "position": pitch, "Velocity": velocity})

    def headYaw(self, yaw, velocity=1):
        assert yaw in range(-5, 6), " headYaw: Yaw needs to be in range -5 to 5"
        requests.post('http://' + self.ip + '/api/head', json={"Axis": "yaw", "position": yaw, "Velocity": velocity})

    def drive(self, linear_velocity, angular_velocity, time):
        # assert linear_velocity in range(-100,101) and angular_velocity in range(-100,101 and time in range(100,3000)), " drive: The velocities needs to be in the range -100 to 100"
        requests.post('http://' + self.ip + '/api/drive',
                      json={"LinearVelocity": linear_velocity, "AngularVelocity": angular_velocity, "TimeMS": time})
        print("bingo")

    def driveTime(self, linear_velocity, angular_velocity, time_in_milli_second):
        assert linear_velocity in range(-100, 101) and angular_velocity in range(-100,
                                                                                 101), " driveTime: The velocities needs to be in the range -100 to 100"
        ...
        r = requests.post('http://' + self.ip + '/api/drive/time',
    ...
    def driveTrack(self, left_track_speed, right_track_speed):
        assert left_track_speed in range(-100, 101) and right_track_speed in range(-100,
                                                                                   101), " driveTrack: The velocities needs to be in the range -100 to 100"
    ...
    def clearLearnedFaces(self):
        requests.post('http://' + self.ip + '/api/faces/clearall')
        self.faces_saved = []
